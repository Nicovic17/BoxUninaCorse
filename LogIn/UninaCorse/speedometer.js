...
var speedGradient = ctx.createLinearGradient(0,0,500,500);
...
function drawSpeedo(speed) {
  ...
  for (var i = 10; i <= topSpeed; i += 10) {
      ...
      drawMiniNeedle(calculateSpeedAngle(i / topSpeed, 133.07888, 38.37) * Math.PI, i % 20 == 0 ? 3 : 1, i%20 == 0 ? i : '');
  }
  ...
  ctx.arc(250, 250, 228, .668 * Math.PI, calculateSpeedAngle(speed / topSpeed, 133.07888, 38.37) * Math.PI);
  ctx.stroke();
  //COLORE E ANGOLATURA DELLA LANCETTA
  ctx.shadowBlur = 0;
  ctx.strokeStyle = '#41dcf4';
  speedNeedle(calculateSpeedAngle(speed / topSpeed, 133.07888, 38.37) * Math.PI);
}